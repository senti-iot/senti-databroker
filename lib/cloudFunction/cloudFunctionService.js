...
class cloudfunctionService {
	...
	async getCloudFunctionById(id, deleted = 0) {
		/**
		...
		let select = `SELECT
					...
					FROM
						cloudFunction cf
					INNER JOIN organisation o on o.id = cf.orgId
					WHERE cf.id=? AND cf.deleted = ?;`
	...
	async getCloudFunctionsByUUIDS(uuids = false) {
		...
		let select = `SELECT
			...
			FROM cloudFunction cf
			INNER JOIN org o on o.id = cf.orgId
	...
	async updateCloudFunction(cloudFunc = false) {
		...
		let dbO = new DbCloudFunction(cloudFunc)
		console.log('cloudFunc -updateCloudFunction', cloudFunc)
		dbO.id = await this.getIdByUUID(cloudFunc.uuid)
		console.log('dbO', dbO)
		let update = `UPDATE cloudFunction
		       SET
				name=?,
				js=?,
				\`type\`=?,
	...
	async deleteCloudFunction(cloudFunc = false) {
		...
		let id = await this.getIdByUUID(cloudFunc.uuid)

		let delSql = `UPDATE cloudFunction
					SET
					deleted=1
					WHERE id=?;`
		let sql = this.db.format(delSql, [id])
		console.log(sql)
