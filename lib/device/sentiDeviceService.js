...
class sentiDeviceService {
	...
	async getDeviceById(id) {
		...
		let select = `SELECT d.id, d.uuid, d.uuname, d.name, d.type_id, d.reg_id, d.metadata, d.lat, d.lng as lon, d.communication, d.description,
					JSON_OBJECT('uuid', r.uuid, 'name', r.name, 'protocol', r.protocol, 'uuname', r.uuname, 'org', JSON_OBJECT('uuname', o.uuname)) as registry,
		...
		rs[0][0].dataKeys = [...rs[0][0].dataKeys.map(k => ({ ...k, origin: 'devicetype' })), rs[0][0].syntheticKeys.map(k => ({ ...k, origin: 'device' }))]
	...
	async getTotalDevices(orgUUID) {
		let selectQuery = `SELECT count(*) as total from device d
						   INNER JOIN deviceType dt on dt.id = d.type_id
						   INNER JOIN organisation o on o.id = dt.orgId
	...
	async getDevicesByUUID(uuids = false) {
		...
		let select = `SELECT
                    ...
                    JSON_OBJECT('uuid', dt.uuid, 'name', dt.name) as deviceType,
                    d.created,
                    IF(ISNULL(tr.resourceUUID) , json_ARRAY(), JSON_ARRAYAGG(if(tr.deleted = 1, JSON_OBJECT(), JSON_OBJECT('uuid', t.uuid, 'name', t.name, 'color', t.color, 'description', t.description)))) as tags
                FROM
                    device d
				LEFT JOIN tagResource tr on tr.resourceUUID = d.uuid AND tr.deleted=0
				LEFT JOIN tag t on t.uuid = tr.tagUUID
	...
	async updateDevice(device = false) {
		...
		let sql = this.db.format(update, [dbO.uuname, dbO.name, dbO.type_id, dbO.reg_id, dbO.description,
			(dbO.metadata !== null) ? JSON.stringify(dbO.metadata) : dbO.metadata,
			dbO.lat, dbO.lng, dbO.address, dbO.locType, dbO.communication, (dbO.dataKeys !== null) ? JSON.stringify(dbO.dataKeys) : null, dbO.id])
		console.log(sql)
