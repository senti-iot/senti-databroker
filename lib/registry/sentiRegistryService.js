...
class sentiRegistryService {
	...
	async updateRegistry(registry = false) {
		...
		let update = `UPDATE registry
						...
						config=?
					  WHERE uuid=?;`
		let sql = await this.db.format(update, [dbReg.name, dbReg.uuname, dbReg.region, dbReg.protocol, dbReg.ca_certificate, dbReg.orgId, dbReg.description, null, dbReg.uuid])
		console.log(sql)
		let rs = await this.db.query(update, [dbReg.name, dbReg.uuname, dbReg.region, dbReg.protocol, dbReg.ca_certificate, dbReg.orgId, dbReg.description, null, dbReg.uuid])
		console.log(dbReg)
