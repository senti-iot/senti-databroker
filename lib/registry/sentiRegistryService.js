...
class sentiRegistryService {
	...
	async getRegistriesByUUID(uuids = false) {
		...
		let select = `SELECT r.id, r.uuid, r.name, r.uuname, r.region, r.protocol, r.ca_certificate, r.orgId, r.created, r.description, r.deleted, r.config, o.uuid as orgUUID, o.name as orgName, (SELECT count(d.uuid) from device d where d.reg_id = r.id and d.deleted = 0) as deviceCount
			FROM registry r
	...
	async getRegistryById(id, deleted = 0) {
		let select = `SELECT r.id, r.uuid, r.name, r.uuname, r.region, r.protocol, r.ca_certificate, r.orgId, r.created, r.description, r.deleted, r.config, (SELECT count(d.uuid) from device d where d.reg_id = r.id and d.deleted = 0) as deviceCount
				FROM registry r
