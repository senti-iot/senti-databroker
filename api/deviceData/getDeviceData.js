	...
	if (verifyAPIVersion(apiVersion)) {
		if (authenticate(authToken)) {
			...
			await mysqlConn.query(query, [deviceID, from, to]).then(async rs => {
				...
				let cleanData = {}
				rawData.forEach(r => {
					cleanData[moment(r.created).utc().format('YYYY-MM-DD HH:mm:ss')] = r.data[type]
				})
				console.log(rawData)
				console.log(cleanData)
				if(nId>0) {
					let cData = await engineAPI.post('/', {nIds: [nId], data: cleanData}).then(rs => { 
						console.log('EngineAPI Status:', rs.status); 
						console.log('EngineAPI Response', rs.data)
