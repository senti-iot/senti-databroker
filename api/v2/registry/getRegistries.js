...
router.get('/v2/registries/:orguuid', async (req, res) => {
	/**
	 * Check if lease is valid
	 */
	let lease = await authClient.getLease(req)
	 ...
	 * Return all resources the user has access
	 */
	let resources = await aclClient.findResources(lease.uuid, req.params.orguuid, sentiAclResourceType.registry, sentiAclPriviledge.registry.read)
	if (resources.length === 0) {
		res.status(404).json([])
		return
	}
