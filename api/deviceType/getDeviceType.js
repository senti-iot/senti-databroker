...
router.get('/:version/:customerID/deviceType/:id', async (req, res, next) => {
	...
	if (verifyAPIVersion(apiVersion)) {
		if (authenticate(authToken)) {
			let query = `SELECT dt.id, dt.name, dt.description, dt.inbound, dt.outbound, dt.metadata, dt.deleted, c.name as customerName, c.ODEUM_org_id as orgId, dt.customer_id FROM Device_type dt
